...
class StreamController extends EventHandler {
  ...
  _loadFragmentOrKey(frag, level, levelDetails, pos, bufferEnd) {
    ...
    if ((frag.decryptdata && frag.decryptdata.uri != null) && (frag.decryptdata.key == null)) {
      ...
      logger.log(`Loading ${frag.sn} of [${levelDetails.startSN} ,${levelDetails.endSN}],level ${level}, currentTime:${pos.toFixed(3)},bufferEnd:${bufferEnd.toFixed(3)},buffer remaining:${(bufferEnd-pos).toFixed(3)}`);
  ...
  */
    ...
    */
    ...
    if (media && media.readyState) {
      ...
      if (!media.paused) {
        ...
        const nextLevelId = this.hls.nextLoadLevel, nextLevel = this.levels[nextLevelId],
  ...
  onFragParsingData(data) {
    ...
    if (fragCurrent &&
      ...
      const drift = LevelHelper.updateFragPTSDTS(level.details, frag, data.startPTS, data.endPTS, data.startDTS, data.endDTS),
        hls = this.hls;
          ...
          if(expectedPlaying) {
            ...
            if(!this.stalled) {
              ...
              if (bufferLen <= jumpThreshold && stalledDuration > config.lowBufferWatchdogPeriod * 1000) {
                ...
                const nextBufferStart = bufferInfo.nextStart, delta = nextBufferStart - currentTime;
                if(nextBufferStart &&
